# Import necessary modules and models for authentication and form handling
...
# Define a custom registration form for user registration
class RegistrationForm(UserCreationForm):
    # Define password fields with labels and input attributes
...
# Define a custom login form for user authentication
class LoginForm(AuthenticationForm):
    username = UsernameField(widget=forms.TextInput(attrs={'autofocus': True, 'class': 'form-control'}))
    password = forms.CharField(label=_("Password"), strip=False, widget=forms.PasswordInput(attrs={'autocomplete':'current-password', 'class':'form-control'}))

# Define a form for capturing user address information
...
# Define a custom password reset form
class CustomPasswordResetForm(PasswordResetForm):
    email = forms.EmailField(label=_("Email"), max_length=254, widget=forms.EmailInput(attrs={'autocomplete':'email', 'class':'form-control'}))

# Define a custom form for setting a new password
